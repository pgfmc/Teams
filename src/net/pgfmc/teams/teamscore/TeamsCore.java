...
import net.pgfmc.teams.playerLogistics.PendingRequest;
import net.pgfmc.teams.playerLogistics.TeamAccept;
import net.pgfmc.teams.playerLogistics.TeamRequest;
		...
		if (attacker.getGameMode() == GameMode.SURVIVAL && target.getGameMode() == GameMode.SURVIVAL) {
			...
			if (ATKP.getData("request") == null && DEFP.getData("request") == null) {
				...
				if (ATK != null && DEF == null) { // if the attacker isnt in a team, but the target is
					...
					attacker.sendMessage("You have joined " + TeamsCore.makePossesive(target.getDisplayName()) + " team!");
